            ...
            if(line.indexOf("=") < 0){
                ...
                if(! nextline.startsWith(">>")) {
                    ...
                    if (nextline.startsWith("AssertionError")) {
                        res += `totaal += 1;\n check = false; try {\n` + line + `\n} catch(error) { \n check = true; \n} \n if(check) {\n console.log('Method: ` + line.replace(/(['"])/g, "\\$1") + `: CORRECT (error thrown).'); correct += 1; \n} else { \nconsole.log('Method: ` + line.replace(/(['"])/g, "\\$1") + `: NOT CORRECT (no error)')\n }; \n`;
                    } else {
                        res += `totaal += 1;\n try { if(deepCompare(` + line + `,` + nextline + `)) { \n console.log('Method: ` + line.replace(/(['"])/g, "\\$1") + `: CORRECT'); correct += 1; \n } else { \n console.log('Method: ` + line.replace(/(['"])/g, "\\$1") + `: NOT CORRECT'); console.log('Expected:\n'); console.log('\t` + nextline.replace(/(['"])/g, "\\$1") + `'); console.log('Got:\n'); console.log('\t' + JSON.stringify(` + line + `));\n} } catch(error) { if(error.name === "ReferenceError") { console.log('Method: ` + line.replace(/(['"])/g, "\\$1") + ` does not exist.'); } else { console.log('Method: ` + line.replace(/(['"])/g, "\\$1") + ` threw a non-expected error.'); }\n}`;
                    }
                    index += 1;
                } else {
                    res += line + ";\n";
