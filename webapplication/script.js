            ...
            if(line.indexOf("=") < 0){
                ...
                if(! nextline.startsWith(">>")) {
                    ...
                    if (nextline.startsWith("AssertionError")) {
                        res += `totaal += 1;\n var check = false; try {\n` + line + `\n} catch(error) { \n check = true; \n} \n if(check) {\n console.log('The method ` + line.replace(/(['"])/g, "\\$1") + ` has CORRECTLY thrown an error.'); correct += 1; \n} else { \nconsole.log('The method ` + line.replace(/(['"])/g, "\\$1") + ` has NOT CORRECTLY thrown an AssertionError')\n }; \n`;
                    } else {
                        res += `totaal += 1;\n try { if(deepCompare(` + line + `,` + nextline + `)) { \n console.log('The method ` + line.replace(/(['"])/g, "\\$1") + ` did SUCCEED'); correct += 1; \n } else { \n console.log('The method ` + line.replace(/(['"])/g, "\\$1") + ` did NOT SUCCEED'); console.log('Expected: ` + nextline.replace(/(['"])/g, "\\$1") + `'); console.log('Got: ' + JSON.stringify(` + line + `));\n} } catch(error) { if(error.name === "ReferenceError") { console.log('The method: ` + line.replace(/(['"])/g, "\\$1") + ` does not exist.'); } else { console.log('The method: ` + line.replace(/(['"])/g, "\\$1") + ` threw a non-expected error.'); }\n}`;
                    }
                    index += 1;
                } else {
                    res += line + ";\n";
